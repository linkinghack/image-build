...
ENV PATH="${PATH}:${NVM_DIR}/current/bin:${NPM_GLOBAL}/bin:${PYTHON_ROOT}/current/bin:${ORIGINAL_PATH}:${DOTNET_ROOT}:${DOTNET_ROOT}/tools:${SDKMAN_DIR}/bin:${SDKMAN_DIR}/candidates/gradle/current/bin:${SDKMAN_DIR}/candidates/java/current/bin:/opt/maven/lts:${GOROOT}/bin:${GOPATH}/bin:${PIPX_BIN_DIR}:/opt/conda/condabin:${JAVA_ROOT}/current/bin:${NODE_ROOT}/current/bin:${PHP_ROOT}/current/bin:${RUBY_ROOT}/current/bin:${MAVEN_ROOT}/current/bin:${HUGO_ROOT}/current/bin:${JUPYTERLAB_PATH}"
...
# Install SDKMAN, OpenJDKs, gradle, maven
RUN bash /tmp/scripts/java-debian.sh 8 "${SDKMAN_DIR}" "${USERNAME}" \
    &&  bash /tmp/scripts/java-debian.sh 11 "${SDKMAN_DIR}" "${USERNAME}" \
    &&  bash /tmp/scripts/java-debian.sh 17 "${SDKMAN_DIR}" "${USERNAME}" \
    && bash /tmp/scripts/java-debian.sh 19 "${SDKMAN_DIR}" "${USERNAME}"

RUN bash /tmp/scripts/maven-debian.sh "latest" "${SDKMAN_DIR}" "${USERNAME}" "true"
...
## Install rust lang support
RUN bash /tmp/scripts/rust.sh "${USERNAME}" "true"
