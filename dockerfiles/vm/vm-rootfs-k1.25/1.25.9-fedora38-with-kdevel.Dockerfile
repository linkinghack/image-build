...
RUN dnf update -y && \
    dnf install -y  systemd-sysv udev redhat-release cloud-init sudo curl gnupg vim net-tools bind-utils conntrack && \
...
RUN apt-get update -y && \
    apt-get install -y --no-install-recommends kubelet=1.25.4-00 kubeadm=1.25.4-00 kubectl=1.25.4-00 && \
    rm -rf /var/lib/apt/lists/* && \
